...
final static int UI_BUTTON_PATTERN_BACK    = 23;
...
final static int UI_SLIDER_BLACK           = 43;
final static int UI_SLIDER_WHITE           = 44;
...
final static int UI_NUMFIELD_PATT_WIDTH    = 81;
final static int UI_NUMFIELD_PATT_HEIGHT   = 82;
final static int UI_BUTTON_LOAD_IMAGE      = 91;
final static int UI_BUTTON_SHOW_DATA       = 92;
final static int UI_BUTTON_SHOW_TEXTILE    = 93;
...
final static int UI_NUMFIELD_PATT_WIDTH_DEFVAL  = 30;
final static int UI_NUMFIELD_PATT_HEIGHT_DEFVAL = 40;
...
int uiPatternPaddingW       =  46;
int uiPatternPaddingH       =  84;
int uiPatternPreviewTop     =  80;

int uiPatternShowIconsY     =  48;
int uiPatternShowIndicW     =  40;
int uiPatternShowIndicH     =   5;
...
UIViewFade       uiViewPattern;
UIDataPattern    uiDataPattern;
UIButtonStatic   uiBtnPatternBack;
...
UIViewSlide      uiToolPanePattern;
UIGroup          uiGroupPatternControls;
UINumField       uiNumPattWidth;
UINumField       uiNumPattHeight;
UISlider         uiSliderBlack;
UISlider         uiSliderWhite;
UIGroup          uiGroupJobDetails;

UIViewSlide      uiToolPane;    // references the current tool pane
...
void uiSetup() {
  ...
  imgSpool          = uiLoadImageAsset("Spool.png");
  imgTime           = uiLoadImageAsset("Time.png");
  imgDropImage      = uiLoadImageAsset("DropImage.png");
  imgShowData       = uiLoadImageAsset("ShowData.png");
  imgShowTextile    = uiLoadImageAsset("ShowTextile.png");
  ...
  uiViewPattern = new UIViewFade(0, 0, uiCanvasSmallWidth, uiCanvasHeight);
  uiViewPattern.setCallbackHandler(appUICallbackHandler);
  uiViewPattern.setBackgroundColor(uiColLight);
  uiMainCanvas.add(uiViewPattern);

  uiDataPattern = new UIDataPattern(uiPatternPaddingW, uiPatternPaddingH, uiViewPattern.w -(uiPatternPaddingW*2), uiViewPattern.h -(uiPatternPaddingH*2));
  uiDataPattern.init(appUICallbackHandler, imgDropImage, imgShowData, imgShowTextile);
  uiDataPattern.setTextileSize(UI_NUMFIELD_PATT_WIDTH_DEFVAL, UI_NUMFIELD_PATT_HEIGHT_DEFVAL);
  uiViewPattern.add(uiDataPattern);

  uiBtnPatternBack = uiCreateBackButton(UI_BUTTON_PATTERN_BACK);
  uiViewPattern.add(uiBtnPatternBack);
  ...
  uiToolPanePattern = new UIViewSlide(width -uiToolPaneWidth, 0, uiToolPaneWidth, height);
  uiToolPanePattern.setClosedPos(width, 0);
  uiToolPanePattern.setBackgroundColor(uiColDark);

  // group for spin selection has already been created for the spin toolpane
  // and it will have the same position within the toolpane so sharing shouldn't be a problem 
  uiToolPanePattern.addToBottom(uiGroupSpinSelection, uiPaddingTop);
  
  uiGroupPatternControls = new UIGroup(uiToolPaneOuterPadding, uiSpinControlsY, uiToolPaneSpin.w() - (2*uiToolPaneOuterPadding), height);
  uiGroupPatternControls.setFrame(uiColLight, 1);
  uiToolPanePattern.addToBottom(uiGroupPatternControls, uiToolPaneElemSpacing);
  
  // TODO use appStrings.get("patternSize")
  UIText uiTextPatternSize = uiCreateText("Size", fontTool, uiColLight, uiToolPaneInnerPadding, 0);
  uiTextPatternSize.vAlign = BOTTOM;
  uiGroupPatternControls.addToBottom(uiTextPatternSize, uiToolPaneElemSpacing); 
  
  // TODO default value should be global constant
  uiNumPattWidth = uiCreateNumField(
    UI_NUMFIELD_PATT_WIDTH, 
    0, 0, 
    uiNumFieldW, uiNumFieldH, 
    UI_NUMFIELD_PATT_WIDTH_DEFVAL, "cm");  
  uiGroupPatternControls.addRightOf(uiNumPattWidth, uiTextPatternSize, uiToolPaneElemSpacing);
  
  UIText uiTextSizeX = uiCreateText("x", fontTool, uiColLight, uiToolPaneInnerPadding, 0);
  uiTextSizeX.vAlign = BOTTOM;
  uiGroupPatternControls.addRightOf(uiTextSizeX, uiNumPattWidth, uiToolPaneElemSpacingS);
  
  // TODO default value should be global constant
  uiNumPattHeight = uiCreateNumField(
    UI_NUMFIELD_PATT_HEIGHT, 
    0, 0, 
    uiNumFieldW, uiNumFieldH, 
    UI_NUMFIELD_PATT_HEIGHT_DEFVAL, "cm");  
  uiGroupPatternControls.addRightOf(uiNumPattHeight, uiTextSizeX, uiToolPaneElemSpacingS);
  
  // TODO use appStrings.get("patternSize")
  UIText uiTextAdjustThickness = uiCreateText("Adjust Yarn Thickness", fontToolBold, uiColLight, uiToolPaneInnerPadding, 0);
  uiGroupPatternControls.addToBottom(uiTextAdjustThickness, uiToolPaneElemSpacing *2); 
  
  // TODO use appStrings.get("patternSize")
  UIText uiTextThicknessBlack = uiCreateText("Black", fontTool, uiColLight, uiToolPaneOuterPadding, 0);
  uiGroupPatternControls.addToBottom(uiTextThicknessBlack, uiToolPaneElemSpacing);
  
  // TODO default value should be global constant
  uiSliderBlack = uiCreateSlider(UI_SLIDER_BLACK, uiSliderX, 0, 80);
  uiGroupPatternControls.addToBottom(uiSliderBlack, uiToolPaneElemSpacingS);

  // TODO use appStrings.get("patternSize")
  UIText uiTextThicknessWhite = uiCreateText("White", fontTool, uiColLight, uiToolPaneOuterPadding, 0);
  uiGroupPatternControls.addToBottom(uiTextThicknessWhite, uiToolPaneElemSpacing);

  // TODO default value should be global constant
  uiSliderWhite = uiCreateSlider(UI_SLIDER_WHITE, uiSliderX, 0, 10);
  uiGroupPatternControls.addToBottom(uiSliderWhite, uiToolPaneElemSpacingS);
  
  int patternControlsH = ceil(uiSliderWhite.y() + uiSliderWhite.h() + (uiToolPaneElemSpacing *1.5));
  uiGroupPatternControls.setSize(uiGroupPatternControls.w(), patternControlsH);
  
  uiGroupJobDetails = new UIGroup(uiToolPaneOuterPadding, uiSpinControlsY, uiToolPaneSpin.w() - (2*uiToolPaneOuterPadding), height);
  uiGroupJobDetails.setFrame(uiColLight, 1);
  uiToolPanePattern.addToBottom(uiGroupJobDetails, uiToolPaneElemSpacing);
  
  // the status indicator and play/pause button were already created for the spin toolpane
  // so we can use those elements (which will be added at the bottom) to set the size of this group accordingly
  uiGroupJobDetails.setSize(uiGroupJobDetails.w(), uiTextStatus.y() - uiGroupJobDetails.y() - uiToolPaneElemSpacing);
  
  UIImage uiIconSpool = new UIImage(imgSpool, uiToolPaneInnerPadding, 0);
  uiGroupJobDetails.addToBottom(uiIconSpool, uiToolPaneElemSpacing);
  
  // TODO use appStrings.get("patternSize")
  UIText uiTextSpools = uiCreateText("2 Spools", fontToolBold, uiColLight, uiToolPaneOuterPadding, 0);
  uiGroupJobDetails.addRightOf(uiTextSpools, uiIconSpool, uiToolPaneElemSpacing);
  // Align vertically with spool icon 
  uiTextSpools.setPos(uiTextSpools.x(), uiIconSpool.y() + uiIconSpool.h() - uiTextSpools.h());
  
  UIImage uiIconTime = new UIImage(imgTime, uiToolPaneInnerPadding, 0);
  uiGroupJobDetails.addToBottom(uiIconTime, uiToolPaneElemSpacing);
  
  // TODO use appStrings.get("patternSize")
  UIText uiTextTime = uiCreateText("30 Minutes", fontToolBold, uiColLight, uiToolPaneOuterPadding, 0);
  uiGroupJobDetails.addRightOf(uiTextTime, uiIconTime, uiToolPaneElemSpacing);
  // Align vertically with watch icon 
  uiTextTime.setPos(uiTextTime.x(), uiIconTime.y());
  
  // the status indicator and play/pause button were already created for the spin toolpane
  // they have the same position within the toolpane so sharing shouldn't be a problem 
  uiToolPanePattern.add(uiTextStatus);
  uiToolPanePattern.add(uiStatusIndicator);
  uiToolPanePattern.add(uiBtnPlayPause);
  ...
  */
  ...
  uiSplash.close();
  uiToolPaneSpin.close();
  uiToolPanePattern.open();
  
  uiToolPane = uiToolPanePattern;

  uiMainCanvas.currView = uiViewPattern;
  uiMainCanvas.mode = UIMainCanvas.VIEW_PATTERN;
}
...
// Returns a newly-created and initialized UI numeric field
UINumField uiCreateNumField(int id, float x, float y, float w, float h, int initValue, String suffix) {
  UINumField uiNumField;
  uiNumField = new UINumField(
    x, y, w, h,
    initValue, suffix,
    fontNumField, fontNumField.getSize(),
    uiColDark, uiColLight 
  );
  uiNumField.setId(id);
  uiNumField.setCallbackHandler(appUICallbackHandler);
  uiNumField.textAlign(CENTER, BOTTOM); // vertically, BOTTOM looks better than CENTER
  return uiNumField;
}
