...
void startSpinning() {
  if (isPatternMode) {
    if (hilo.isConnected() && !hilo.isSpinning() && uiDataPattern.hasSource()) {
      isStopping = false;
      timePixelStarted = timeCurr;
      sendCurrentPixel();
    }
    else if (hilo.isSpinning()) {
      debugMessage("startSpinning() WARNING: Hilo is already spinning");
    }
    else {
      debugMessage("startSpinning() WARNING: No image loaded into pattern");
    }
  }
  else {
