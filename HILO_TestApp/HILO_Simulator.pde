...
class HILOSimulator extends HILOStatus implements HILOInterface {
    ...
    switch (state) {
      case HILOStatus.DISCONNECTED:
        ...
        if (timerConnect != null) {
          println("HILOSimulator.connect() WARNING: cancelling existing connect timer");
          timerConnect.cancel();
          timerConnect.purge();
        }
  ...
  boolean disconnect() {
    if (state == HILOStatus.CONNECTING || state == HILOStatus.READY || state == HILOStatus.SPINNING) {
      ...
      if (timerConnect != null) {
        println("HILOSimulator.disconnect() cancelling existing connect timer");
        timerConnect.cancel();
        timerConnect.purge();
        timerConnect = null;
       }
  ...
  boolean setDeliverySpeed(int speed) {
    if (state != READY) {
      println("HILOSimulator.setDeliverySpeed(): can't change speed while disconnected or spinning");
      return false;
    }
  
    println("HILOSimulator.setDeliverySpeed(): setting delivery speed to " + speed);
    deliverySpeed = speed;
    return true; 
  }
  ...
  boolean setSpindleSpeed(int speed) { 
    if (!isConnected()) {
      println("HILOSimulator.setSpindleSpeed(): can't change speed while disconnected");
      return false;
    }
  
    println("HILOSimulator.setSpindleSpeed(): setting spindle speed to " + speed);
    spindleSpeed = speed;
    return true;
  }
  ...
  boolean setElevatorSpeed(int speed) {
    if (!isConnected()) {
      println("HILOSimulator.setElevatorSpeed(): can't change speed while disconnected");
      return false;
    }
    
    println("HILOSimulator.setElevatorSpeed(): setting elevator speed to " + speed);
    elevatorSpeed = speed;
    return true;
  }
  ...
  boolean spin(int draftingSpeedPerc) {
    ...
    if (draftingSpeedPerc < 0 || draftingSpeedPerc > 100) {
      println("HILOSimulator.sendPixel() WARNING: percentage value " + draftingSpeedPerc + " is out of bounds, correcting");
