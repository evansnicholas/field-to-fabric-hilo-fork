...
class HILODevice extends HILOStatus implements HILOInterface {
      ...
      public void run() { 
        while(true) {
          ...
          try {
            ...
            println("threadHiloUpdate.run(): interrupted while sleeping ");
          }
        }
      }
    };
  ...
  boolean connect(String portName) {
    switch (state) {
      case HILOStatus.DISCONNECTED:
        ...
        println("HILODevice.connect(): connecting to HILO on serial port <" + portName + ">");
        
        try {
          serial = new Serial(parent, portName, 115200);
        } catch (RuntimeException rte) {
          state = HILOStatus.ERROR;
          println("HILODevice.connect() ERROR: serial port doesn't exist or is busy <" + portName + ">");
          return false;
        }
        if (!serial.active()) {
          state = HILOStatus.ERROR;
          println("HILODevice.connect() ERROR: can't connect to HILO on serial port <" + portName + ">");
          return false;
        }
        ...
        println("HILODevice.connect(): CONNECTED");
      ...
      case HILOStatus.CONNECTING:
        println("HILODevice.connect() WARNING: already connected on serial port <" + portName + ">");
        return true;
      case HILOStatus.ERROR:
        println("HILODevice.connect() WARNING: a previous error is still flagged; please call disconnect() to clear it");
        return false;
      default:
        return true;
    }
  ...
  boolean disconnect() { 
    if (state == HILOStatus.DISCONNECTED) {
      println("HILODevice.disconnect() WARNING: device was already disconnected");
    } 
    else if (state == HILOStatus.ERROR) {
      println("HILODevice.disconnect(): clearing error status");
    }
    
   
    if (serial != null && serial.active()) {
      println("HILODevice.disconnect(): clearing command queue");
      commandQueue.clear();
      println("HILODevice.disconnect(): disposing of serial port connection");
      serial.dispose();
      serial = null;
    }
  ...
  boolean setDeliverySpeed(int speed) { 
    ...
    println("HILODevice.setDeliverySpeed(): setting delivery speed to " + speed);
    deliverySpeed = speed;
  ...
  boolean setSpindleSpeed(int speed) { 
    if (!isConnected()) {
      println("HILODevice.setSpindleSpeed(): can't change speed while disconnected");
      return false;
    }
    
    // set the spindle speed locally and queue the command to update the actual machine
    println("HILODevice.setSpindleSpeed(): setting spindle speed to " + speed);
    spindleSpeed = speed;
  ...
  boolean setElevatorSpeed(int speed) {
    if (!isConnected()) {
      println("HILODevice.setElevatorSpeed(): can't change speed while disconnected");
      return false;
    }
    
    // set the elevator speed locally and queue the command to update the actual machine
    println("HILODevice.setElevatorSpeed(): setting elevator speed to " + speed);
    elevatorSpeed = speed;
  ...
  boolean spin(int draftingSpeedPerc) {
    ...
    if (draftingSpeedPerc < 0 || draftingSpeedPerc > 100) {
      println("HILODevice.spin() WARNING: percentage value " + draftingSpeedPerc + " is out of bounds, correcting");
  ...
  protected void queueCommand(char command) {
    println("HILODevice.queueCommand(): " + command);
  ...
  protected void queueCommand(char command, int value) {
    println("HILODevice.queueCommand(): " + command + " " + value);
  ...
  protected void queueCommand(char command, int valueMin, int valueMax) {
    println("HILODevice.queueCommand(): " + command + " " + valueMin + " " + valueMax);
  ...
  protected void processCommandQueue() {
    ...
    println("HILODevice.processCommandQueue(): processing command " + command.command);
    ...
    println("HILODevice.processCommandQueue(): sending command " + strCommand);
    if (serial.active()) {
      serial.write(strCommand);
    }
    else {
      println("HILODevice.processCommandQueue() WARN: can't send command " + strCommand + " - serial port closed");
    }
  ...
  protected void processSerialEvents() {
    ...
    while (serial.available() > 0) {
      ...
      if (commandBufferPos >= commandBuffer.length) {
        println("HILODevice.processSerialEvents() ERROR: command buffer is full, clearing");
        println("\tcommand buffer:");
        println("\t" + new String(commandBuffer));
        commandBufferPos = 0;
      }
  ...
  protected void handleMessagePing() {
    if (state == HILOStatus.READY) {
      println("HILODevice.handleMessagePing(): received PING but HILO was already connected!");
    }
    else {
      println("HILODevice.handleMessagePing() WARNING: received PING but HILO state is [" + state + "]");
    }
  }
  ...
  private void handleMessageSpin() {
    if (currentCommand != CMD_RUN) {
      println("HILODevice.handleMessageSpin() WARNING: received " + CMD_RUN + " but current command is " + currentCommand);
    }
  ...
  private void handleMessageStop() {
    if (currentCommand != CMD_STOP) {
      println("HILODevice.handleMessageStop() WARNING: received " + CMD_STOP + " but current command is " + currentCommand);
    }
    state = HILOStatus.READY;
  ...
  private void handleMessageSettings(int command) {
    if (state == HILOStatus.SPINNING && command != CMD_SPINDLE_SPEED) {
      println("HILODevice.handleMessageSettings() WARNING: received " + command + " but current state is HILOStatus.SPINNING");
    }
    
    currentCommand = CMD_NONE;
    println("HILODevice.handleMessageSettings() confirmed " + (char)command);
  }
  
  
  // Handles a debug message sent from the machine.
  private void handleMessageDebug(String completeMessage) {
    println("HILODevice.handleMessageDebug() HILO DEBUG: " + completeMessage);
  }
  
  
  // Handles an error message sent by the machine.
  private void handleMessageError(int errorCode) {
    println("HILODevice.handleMessageError() WARNING: not implemented!");
  }
  ...
  int parseHiloMessage(String message) {
    println("parseHiloMessage(): parsing message " + message);
    
    if (message.charAt(0) != CMD_DELIM_BEGIN) {
      println("parseHiloMessage() ERROR: message doesn't start with delimiter");
    }
    else if (message.charAt(message.length()-1) != CMD_DELIM_END) {
      println("parseHiloMessage() ERROR: message doesn't end with delimiter");
    }
    ...
    if (message.charAt(1) == CMD_ERROR) {
      ...
      try {
        ...
        println("parseHiloMessage() Error code: " + errorCode);
      } catch (NumberFormatException nfe) {
        println("parseHiloMessage() ERROR: couldn't parse error code from HILO device, message " + message);
        return CMD_ERROR;
      }
    }
    ...
    if (message.charAt(1) == CMD_DEBUG) {
      ...
      println("parseHiloMessage() Debug Message: " + debugMessage);
    }
